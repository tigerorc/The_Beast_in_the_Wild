...
################################################################################
## 对话界面 ##
## 此界面必须创建一个 id 为 what 的文本可视控件，因为 Ren'Py 使用它来管理文本显示。
## 它还可以创建 id 为 who 和 id 为 window 的可视控件来应用样式属性。
...
#            textbutton _("快存") action QuickSave()
            textbutton _("快进") action QuickLoad()
...
screen navigation():
    ...
    vbox:
        ...
        if main_menu:
            textbutton _("{outlinecolor=#ebf1f5}{alpha=1}新开始{/alpha}{/outlinecolor}") action ShowMenu("choseChapters1")#action Start()
        else:
            textbutton _("历史") action ShowMenu("history")
            textbutton _("保存") action ShowMenu("save")
        #同样的色号，下面比上面亮一点，背景衬托，因此需要修改透明度
        #描边，增强显示效果
        textbutton _("{outlinecolor=#ebf1f5}{alpha=0.8}旧回忆{/alpha}{/outlinecolor}") action ShowMenu("load")
        if _in_replay:
    ...
    vbox:
        ...
        if main_menu:
            if renpy.variant("pc"):
            #退出按钮 仅PC端
                textbutton _("{alpha=0.8}{size=-10}退出{/size}{/alpha}") action Quit(confirm=not main_menu)
    ...
    vbox:
        ...
        ypos 140
        yalign 0.5
        textbutton _("{alpha=1}设置{/alpha}") action ShowMenu("preferences")
        textbutton _("{alpha=1}关于{/alpha}") action ShowMenu("about")
